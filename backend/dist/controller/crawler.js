...
export async function crawlPage(sitemapUrl) {
    ...
    const crawler = new PlaywrightCrawler({
        ...
        async requestHandler({ request, page, enqueueLinks, log }) {
            try {
                ...
                await Page.findOneAndUpdate({ url }, { url, outgoingLinks: uniqueOutgoing, lastCrawledAt: new Date() }, { upsert: true, new: true });
                for (const link of uniqueOutgoing) {
                    await Page.findOneAndUpdate({ url: link }, { $addToSet: { incomingLinks: url } }, { upsert: true });
                }
